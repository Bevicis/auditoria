...
def auditar_url(url):
    ...
    try:
        ...
        script_data = " ".join([(s.get("id", "") or "") + (s.get("src", "") or "") + (s.string or "") for s in scripts if s])
        html_total = html + script_data.lower()
        banner_ok = any(g in html_total for g in gestores)
        resultado.append((
